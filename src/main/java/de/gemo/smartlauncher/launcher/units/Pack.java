...
public class Pack {
    ...
    public boolean extractPack() {
        try {
            ...
            InputStream inputStream = new FileInputStream(new File(VARS.DIR.PACKS + "/" + this.getPackName(), this.getPackName() + "-" + GameLauncher.getPackInfo().getPackVersion() + ".zip"));
    ...
    public boolean handlePackJson(File file) {
        try {
            ...
            GameLauncher.getPackInfo().setGameVersion(jsonValue.asString().replaceAll("\"", ""));

            // override mainClass...
            jsonValue = json.get("mainClass");
            if (jsonValue != null) {
                GameLauncher.getPackInfo().overrideMainClass(jsonValue.toString());
            }

            // override mcArguments
            jsonValue = json.get("minecraftArguments");
            if (jsonValue != null) {
                GameLauncher.getPackInfo().overrideMCArguments(jsonValue.toString());
            }
